...
namespace XRMultiplayer
{
    ...
    public class XRAvatarVisuals : MonoBehaviour
    {
        ...
        public virtual void PlayerSpawnedLocal()
        {
            ...
            int localAvatarLayer = LayerMask.NameToLayer("SelfAvatarNetworked");
            foreach (var r in m_HeadRends)
            {
                if (r == null)
                    continue;

                if (localAvatarLayer != -1)
                    r.gameObject.layer = localAvatarLayer;

                // Keep local-player head visuals out of first-person view while preserving shadows.
                r.shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.ShadowsOnly;
            }

            if (m_HostVisuals != null)
            {
                m_HostVisuals.SetActive(false);
            }
        }
        ...
        public virtual void PlayerSpawnedAll()
        {
            ...
            if (m_NetworkPlayer != null && m_NetworkPlayer.IsOwner)
            {
                m_HostVisuals.SetActive(false);
                return;
            }
